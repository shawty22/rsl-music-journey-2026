#!/usr/bin/env node
// Merges data/enrichment/wave2/output/batch-*.json into data/normalized/artists.json.
...
const OUT_DIR = path.join(ROOT, "data", "enrichment", "wave2", "output");
...
const batchFiles = readdirSync(OUT_DIR).filter((f) => /^batch-\d+\.json$/.test(f)).sort((a, b) => {
...
for (const file of batchFiles) {
  ...
  for (const r of records) {
    ...
    existing.research_confidence = "wave2-pass";
