...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .search-sticky { position: sticky; top: 41px; z-index: 40; background: var(--bg); padding: 14px 0 8px; margin-top: 20px; }
  #search-box { width: 100%; padding: 12px 14px; background: var(--surface); border: 1px solid var(--line); border-radius: 10px; color: var(--text); font-size: 15px; }
  #search-status { font-size: 12px; color: var(--text-faint); margin-top: 8px; min-height: 16px; }
...
<body>
  ...
  <div class="search-sticky">
    <input id="search-box" type="text" placeholder="Search artist or genre…" oninput="filterEntries(this.value)" />
  ...
  <script>
    function filterEntries(q) {
      ...
      // Full profiles and repeat-performance backreferences both carry the
      // artist's name — match on the visible text of each independently so
      // a hit in one doesn't wrongly keep the other visible.
      document.querySelectorAll('.artist-entry, .backref').forEach(function (el) {
        var text = el.textContent.toLowerCase();
        el.style.display = !q || text.indexOf(q) !== -1 ? '' : 'none';
      });

      // A day with nothing visible left in it is just clutter while
      // searching — collapse the whole slot/day rather than showing an
      // empty time heading or a day header over nothing.
      document.querySelectorAll('.time-slot').forEach(function (slot) {
        var anyVisible = Array.prototype.some.call(
          slot.querySelectorAll('.artist-entry, .backref'),
          function (el) { return el.style.display !== 'none'; }
        );
        slot.style.display = !q || anyVisible ? '' : 'none';
      });
      var totalMatches = 0;
      document.querySelectorAll('.day-section').forEach(function (day) {
        var visibleSlots = Array.prototype.filter.call(day.querySelectorAll('.time-slot'), function (s) { return s.style.display !== 'none'; }).length;
        day.style.display = !q || visibleSlots > 0 ? '' : 'none';
        totalMatches += visibleSlots ? Array.prototype.filter.call(day.querySelectorAll('.artist-entry'), function (el) { return el.style.display !== 'none'; }).length : 0;
      });

      if (!q) status.textContent = '';
      else if (totalMatches === 0) status.textContent = 'No matches for "' + q + '" — try a broader term.';
      else status.textContent = totalMatches + ' artist' + (totalMatches === 1 ? '' : 's') + ' match "' + q + '".';
    }
  </script>
</body>
</html>
